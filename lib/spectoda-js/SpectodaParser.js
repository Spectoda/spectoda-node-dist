...
class TnglCompiler {
    ...
    compileVariableAddress(variable_reference) {
        Logging_js_1.logging.verbose(`compileVariableAddress(${variable_reference})`);
        ...
        Logging_js_1.logging.verbose(`VALUE_ADDRESS name=${variable_name}, address=${variable_address}`);
    ...
    compileConstVariableDeclaration(variable_declaration) {
        Logging_js_1.logging.verbose(`compileConstVariableDeclaration(${variable_declaration})`);
        let reg = variable_declaration.match(/const +([A-Za-z_][\w]*) *=/);
        if (!reg) {
            Logging_js_1.logging.error("Failed to compile variable declaration");
            return;
        }
        const variable_name = reg[1];
        ...
        Logging_js_1.logging.verbose(`DECLARE_VARIABLE name=${variable_name} address=${variable_address}`);
        ...
        switch (word) {
            ...
            default:
                ...
                if (possible_variable_address !== undefined) {
                    Logging_js_1.logging.debug(`VARIABLE_READ name=${word}, address=${possible_variable_address}`);
                    this.#tnglWriter.writeFlag(TNGL_FLAGS.VARIABLE_READ);

...
exports.TnglCodeParser = exports.TnglCompiler = void 0;
...
class TnglCompiler {
    ...
    #var_declarations;
    constructor() {
        this.#tnglWriter = new TnglWriter_js_1.TnglWriter();
    ...
    compileByte(byte) {
        ...
        if (!reg) {
            Logging_js_1.logging.error("Failed to compile a byte");
            return;
        }
    ...
    compileChar(char) {
        ...
        if (!reg) {
            Logging_js_1.logging.error("Failed to compile char");
            return;
        }
        if (reg[1] === "-") {
    ...
    compileString(string) {
        ...
        if (!reg) {
            Logging_js_1.logging.error("Failed to compile a string");
            return;
        }
    ...
    compileInfinity(infinity) {
        ...
        if (!reg) {
            Logging_js_1.logging.error("Failed to compile a infinity");
            return;
        }
        if (reg[1] === "Infinity" || reg[1] === "+Infinity") {
            ...
            Logging_js_1.logging.error("Error while compiling infinity");
        }
    }
    compileValueAddress(variable_reference) {
        Logging_js_1.logging.verbose(`compileValueAddress(${variable_reference})`);
        let reg = variable_reference.match(/&([a-z_][\w]*)/i);
        if (!reg) {
            Logging_js_1.logging.error("Failed to compile variable address");
            return;
        }
        ...
        Logging_js_1.logging.verbose(`VALUE_ADDRESS name=${variable_name}, address=${value_address}`);
    ...
    compileColor(color) {
        ...
        if (!reg) {
            Logging_js_1.logging.error("Failed to compile color");
            return;
        }
    ...
    compilePercentage(percentage) {
        ...
        if (!reg) {
            Logging_js_1.logging.error("Failed to compile percentage");
            return;
        }
        ...
        if (val > -UNIT_ERROR && val < UNIT_ERROR) {
            ...
            const remapped = (0, functions_js_1.mapValue)(val, -100.0, 100.0, PERCENTAGE_MIN, PERCENTAGE_MAX);
    ...
    compileLabel(label) {
        ...
        if (!reg) {
            Logging_js_1.logging.error("Failed to compile a label");
            return;
        }
    ...
    compilePixels(pixels) {
        ...
        if (!reg) {
            Logging_js_1.logging.error("Failed to compile pixels");
            return;
        }
    ...
    compileConstDeclaration(variable_declaration) {
        Logging_js_1.logging.verbose(`compileConstDeclaration("${variable_declaration}")`);
        let reg = variable_declaration.match(/const +([A-Za-z_][\w]*) *=/);
        if (!reg) {
            Logging_js_1.logging.error("Failed to compile const declaration");
            return;
        }
        ...
        Logging_js_1.logging.debug(`DECLARE_VALUE_ADDRESS name=${const_name} address=${const_address}`);
    ...
    compileLetDeclaration(variable_declaration) {
        Logging_js_1.logging.verbose(`compileLetDeclaration(${variable_declaration})`);
        let reg = variable_declaration.match(/let +([A-Za-z_][\w]*) *=/);
        if (!reg) {
            Logging_js_1.logging.error("Failed to compile let declaration");
            return;
        }
        ...
        Logging_js_1.logging.debug(`DECLARE_VALUE_ADDRESS name=${let_name} address=${let_address}`);
    ...
    compileVarDeclaration(variable_declaration) {
        Logging_js_1.logging.verbose(`compileVarDeclaration(${variable_declaration})`);
        let reg = variable_declaration.match(/var +([A-Za-z_][\w]*) *=/);
        if (!reg) {
            Logging_js_1.logging.error("Failed to compile var declaration");
            return;
        }
        ...
        Logging_js_1.logging.debug(`DECLARE_VALUE_ADDRESS name=${var_name} address=${var_address}`);
        ...
        switch (word) {
            ...
            default:
                ...
                if (var_address !== undefined) {
                    Logging_js_1.logging.debug(`VALUE_READ_ADDRESS name=${word}, address=${var_address}`);
                    this.#tnglWriter.writeFlag(TNGL_FLAGS.VALUE_READ_ADDRESS);
                    this.#tnglWriter.writeUint16(var_address);
                    break;
                }
                // === unknown ===
                Logging_js_1.logging.warn("Unknown word >", word, "<");
                break;
        }
    }
    ...
    compileConnection(connection) {
        ...
        if (match === null) {
            Logging_js_1.logging.error("Failed to compile connection");
            return;
        }
        ...
        if (origin_value_address === undefined) {
            Logging_js_1.logging.error(`Failed to find origin variable address [${origin}]`);
            return;
        }
        if (destination_variable_address === undefined) {
            Logging_js_1.logging.error(`Failed to find destination variable address [${destination}]`);
            return;
        }
...
exports.TnglCompiler = TnglCompiler;
class TnglCodeParser {
    ...
    parseTnglCode(tngl_code) {
        Logging_js_1.logging.verbose(tngl_code);
        this.#compiler.reset();
        const tokens = this.#tokenize(tngl_code, TnglCodeParser.#parses);
        Logging_js_1.logging.verbose(tokens);
        for (let index = 0; index < tokens.length; index++) {
            ...
            switch (element.type) {
                ...
                case "percentage":
                    ...
                    break;
                case "float":
                    Logging_js_1.logging.error('"Naked" float numbers are not permitted.');
                    break;
                case "number":
                    Logging_js_1.logging.error('"Naked" numbers are not permitted.');
                    break;
                // case "arrow":
                //   // skip
                //   break;
                ...
                case "connection":
                    ...
                    break;
                default:
                    Logging_js_1.logging.warn("Unknown token type >", element.type, "<");
                    break;
            }
        }
        this.#compiler.compileFlag(TNGL_FLAGS.END_OF_TNGL_BYTES);
        let tnglBytes = this.#compiler.tnglBytes;
        Logging_js_1.logging.verbose(tnglBytes);
        // logging.debug(uint8ArrayToHexString(tnglBytes));
        Logging_js_1.logging.info("Compiled tnglbytes length:", tnglBytes.length);
        return tnglBytes;
    }
    static #parses = {
        connection: /[\w]+->\[\w*\][\w]+\s*;/,

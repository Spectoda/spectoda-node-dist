...
class Spectoda {
    ...
    constructor(connectorType = "default", reconnectionInterval = 1000, runOnServer = false) {
        ...
        this.#ownerKey = null;
        this.interface = new SpectodaInterfaceLegacy_js_1.SpectodaInterfaceLegacy(this, reconnectionInterval);
        if (connectorType) {
        ...
        this.interface.onConnected = event => {
            if (!this.#adopting) {
                ...
                Logging_js_1.logging.verbose("connected event skipped because of adopt");
            }
        };
        this.interface.onDisconnected = event => {
            if (!this.#adopting) {
                Logging_js_1.logging.info("> Device disconnected");
                this.interface.emit("disconnected", { target: this });
            }
            else {
                Logging_js_1.logging.verbose("disconnected event skipped because of adopt");
            }
        };
    ...
    connectRemoteControl() {
        ...
        Logging_js_1.logging.debug("> Connecting to Remote Control");
        if (!this.socket) {
            ...
            this.socket.on("connect", () => {
                Logging_js_1.logging.debug("> Connected to remote control");
                window.alert((0, i18n_js_1.t)("Connected to remote control"));
            });
            this.socket.on("disconnect", () => {
                Logging_js_1.logging.debug("> Disconnected from remote control");
    ...
    disconnectRemoteControl() {
        Logging_js_1.logging.debug("> Disonnecting from the Remote Control");
        this.#reconnectRC = false;
    ...
    adopt(newDeviceName = null, newDeviceId = null, tnglCode = null, ownerSignature = null, ownerKey = null, autoSelect = false) {
        ...
        return ((autoSelect ? this.interface.autoSelect(criteria, 4000) : this.interface.userSelect(criteria, 60000))
            ...
            Logging_js_1.logging.debug("> Adopting device...");
            Logging_js_1.logging.verbose(bytes);
            return this.interface
                .request(bytes, true)
                .then(response => {
                let reader = new TnglReader_js_1.TnglReader(response);
                Logging_js_1.logging.debug("> Got response:", response);
                ...
                Logging_js_1.logging.verbose(`error_code=${error_code}, device_mac=${device_mac}`);
                if (error_code === 0) {
                    ...
                    window.alert((0, i18n_js_1.t)("Zkuste to, prosím, později."), (0, i18n_js_1.t)("Přidání se nezdařilo"), { confirm: (0, i18n_js_1.t)("OK") });
    ...
    connect(devices = null, autoConnect = true, ownerSignature = null, ownerKey = null, connectAny = false, fwVersion = "", infiniteReconnection = false) {
        Logging_js_1.logging.info(`connect(devices=${devices}, autoConnect=${autoConnect}, ownerSignature=${ownerSignature}, ownerKey=${ownerKey}, connectAny=${connectAny}, fwVersion=${fwVersion}, infiniteReconnection=${infiniteReconnection})`);
        ...
        return (autoConnect ? this.interface.autoSelect(criteria) : this.interface.userSelect(criteria))
            ...
            if (!this.#infiniteReconnection) {
                Logging_js_1.logging.warn("Skipping error alerting");
                return;
            }
    ...
    syncTngl(tngl_code, tngl_bytes = null, tngl_bank = 0) {
        Logging_js_1.logging.verbose("syncTngl()");
    ...
    writeTngl(tngl_code, tngl_bytes = null, memory_bank = 0) {
        Logging_js_1.logging.verbose("writeTngl()");
    ...
    emitPercentageEvent(event_label, event_value, device_ids = [0xff], force_delivery = false) {
        Logging_js_1.logging.info(`emitPercentageEvent(label=${event_label},value=${event_value},id=${device_ids},force=${force_delivery})`);
    ...
    syncClock() {
        Logging_js_1.logging.debug("> Syncing clock from device");
        return this.interface.syncClock().then(() => {
            Logging_js_1.logging.debug("> App clock synchronized");
        });
    }
    // TODO add
    syncState(deviceId) {
        Logging_js_1.logging.debug("> Synchronizing state...");
        const request_uuid = this.#getUUID();
    ...
    updateNetworkFirmware(firmware) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            try {
                ...
                Logging_js_1.logging.debug("Rebooting whole network...");
    ...
    async updatePeerFirmware(peer) {
        ...
        return this.interface.request(bytes, true).then(response => {
            let reader = new TnglReader_js_1.TnglReader(response);
            Logging_js_1.logging.info(`response.byteLength=${response.byteLength}`);
    ...
    readDeviceConfig(mac = "ee:33:fa:89:08:08") {
        Logging_js_1.logging.debug("> Reading device config...");
        const request_uuid = this.#getUUID();
    ...
    updateDeviceConfig(config) {
        Logging_js_1.logging.debug("> Updating config...");
    ...
    updateNetworkConfig(config) {
        Logging_js_1.logging.debug("> Updating config of whole network...");
        const encoder = new TextEncoder();
        const config_bytes = encoder.encode(config);
        const config_bytes_size = config.length;
        // make config update request
        ...
        return this.interface.execute(request_bytes, "CONF").then(() => {
            Logging_js_1.logging.debug("> Rebooting network...");
    ...
    requestTimeline() {
        Logging_js_1.logging.debug("> Requesting timeline...");
        const request_uuid = this.#getUUID();
    ...
    rebootDevice() {
        Logging_js_1.logging.debug("> Rebooting device...");
        const payload = [SpectodaInterfaceLegacy_js_1.COMMAND_FLAGS.FLAG_DEVICE_REBOOT_REQUEST];
        return this.interface.request(payload, false);
    }
    rebootAndDisconnectDevice() {
        Logging_js_1.logging.debug("> Rebooting and disconnecting device...");
    ...
    removeOwner() {
        Logging_js_1.logging.debug("> Removing owner...");
        const request_uuid = this.#getUUID();
    ...
    removeNetworkOwner() {
        Logging_js_1.logging.debug("> Removing network owner...");
        const request_uuid = this.#getUUID();
        const bytes = [SpectodaInterfaceLegacy_js_1.COMMAND_FLAGS.FLAG_ERASE_OWNER_REQUEST, ...(0, functions_js_1.numberToBytes)(request_uuid, 4)];
        return this.interface.execute(bytes, true);
    }
    getFwVersion() {
        Logging_js_1.logging.debug("> Requesting fw version...");
        const request_uuid = this.#getUUID();
        ...
        Logging_js_1.logging.info("getFwVersion", { bytes });
        return this.interface.request(bytes, true).then(response => {
            let reader = new TnglReader_js_1.TnglReader(response);
            Logging_js_1.logging.info(`response.byteLength=${response.byteLength}`);
    ...
    getTnglFingerprint(tngl_bank = 0) {
        Logging_js_1.logging.debug("> Getting TNGL fingerprint...");
    ...
    setNetworkDatarate(datarate) {
        Logging_js_1.logging.debug(`> Setting network datarate to ${datarate} bsp...`);
        const request_uuid = this.#getUUID();
    ...
    readRomPhyVdd33() {
        Logging_js_1.logging.debug("> Requesting rom_phy_vdd33 ...");
        const request_uuid = this.#getUUID();
    ...
    readPinVoltage(pin) {
        Logging_js_1.logging.debug(`> Requesting pin ${pin} voltage ...`);
        const request_uuid = this.#getUUID();
    ...
    getConnectedPeersInfo() {
        Logging_js_1.logging.debug("> Requesting connected peers info...");
        const request_uuid = this.#getUUID();
    ...
    readEventHistory() {
        Logging_js_1.logging.debug("> Requesting event history bytecode...");
        const request_uuid = this.#getUUID();
    ...
    eraseEventHistory() {
        Logging_js_1.logging.debug("> Erasing event history...");
        const request_uuid = this.#getUUID();
    ...
    deviceSleep() {
        ...
        Logging_js_1.logging.debug("> Sleep device...");
        const request_uuid = this.#getUUID();
    ...
    saveState() {
        throw "WorkInProgress";
        Logging_js_1.logging.debug("> Saving state...");
        const request_uuid = this.#getUUID();
    ...
    getControllerInfo() {
        Logging_js_1.logging.debug("> Requesting controller info ...");
        const request_uuid = this.#getUUID();

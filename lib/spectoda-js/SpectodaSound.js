...
class SpectodaSound {
    ...
    constructor() {
        ...
        this.#events = (0, functions_js_1.createNanoEvents)();
        this.#rmsMax = 0;
        this.#rmsMin = 0;
    }
    /**
        ...
        if (!mediaStream || mediaStream === "microphone") {
            ...
            if (navigator.mediaDevices) {
                ...
                await new Promise((resolve, reject) => {
                    navigator.mediaDevices
                        ...
                        resolve();
                        Logging_js_1.logging.debug("SpectodaSound.connect", "Connected microphone");
                    })
                        .catch(e => {
                        reject(e);
            ...
            await new Promise(async (resolve, reject) => {
                const srcObject = await navigator.mediaDevices
                    ...
                    this.#source = this.#audioContext.createMediaStreamSource(this.#stream);
                    resolve();
                    Logging_js_1.logging.debug("SpectodaSound.connect", "Connected SystemSound");
                })
                    .catch(e => {
                    reject(e);
            ...
            Logging_js_1.logging.debug("SpectodaSound.connect", "Connected mediaStream");
    ...
    async start() {
        ...
        if (!this.running) {
            ...
            this.#fft = new dsp_js_1.FFT(this.BUFF_SIZE, this.#audioContext.sampleRate);
    ...
    async autoEmitFunctionValue(func) {
        ...
        if (data) {
            ...
            if (this.running) {
                (0, functions_js_1.sleep)(10).finally(() => this.autoEmitFunctionValue(func));
            }
        }
    }
    setBuffSize(size) {
    ...
    processHandler(e) {
        ...
        var out = (0, functions_js_1.mapValue)(rms_loudness_spectrum, this.#rmsMin, this.#rmsMax, 0.0, 100.0);
        // console.log(
...
exports.SpectodaSound = SpectodaSound;

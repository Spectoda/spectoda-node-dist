...
exports.FlutterConnector = void 0;
...
class FlutterConnection {
    constructor() {
        ...
        if (window.flutterConnection) {
            Logging_js_1.logging.debug("FlutterConnection already inited");
            return;
        }
        Logging_js_1.logging.debug("Initing FlutterConnection");
        // @ts-ignore
        ...
        //   });
        if (this.available()) {
            Logging_js_1.logging.debug("Flutter Connector available");
            window.addEventListener("#resolve", e => {
                // @ts-ignore
                const value = e.detail.value;
                Logging_js_1.logging.debug(`Triggered #resolve: [${value}]`);
                // @ts-ignore
            ...
            window.addEventListener("#reject", e => {
                // @ts-ignore
                const value = e.detail.value;
                Logging_js_1.logging.debug(`Triggered #reject: [${value}]`);
                // @ts-ignore
            ...
            window.addEventListener("#emit", e => {
                ...
                Logging_js_1.logging.debug(`Triggered #emit: ${event}`, event);
                // @ts-ignore
            ...
            window.addEventListener("#process", e => {
                ...
                Logging_js_1.logging.debug(`Triggered #process: [${bytes}]`, bytes);
                // @ts-ignore
            ...
            window.addEventListener("#network", e => {
                // @ts-ignore
                const bytes = e.detail.value;
                Logging_js_1.logging.debug(`Triggered #network: [${bytes}]`, bytes);
                // @ts-ignore
                window.flutterConnection.process(bytes);
            });
            window.addEventListener("#device", e => {
                // @ts-ignore
                const bytes = e.detail.value;
                Logging_js_1.logging.debug(`Triggered #device: [${bytes}]`, bytes);
            });
            window.addEventListener("#clock", e => {
                // @ts-ignore
                const bytes = e.detail.value;
                Logging_js_1.logging.debug(`Triggered #clock: [${bytes}]`, bytes);
            });
        }
        else {
            Logging_js_1.logging.debug("flutter_inappwebview in window NOT detected");
            Logging_js_1.logging.info("Simulating Flutter Functions");
                    ...
                    case "userSelect": // params: (criteria_json, timeout_number)
                        {
                            ...
                            await (0, functions_js_1.sleep)(Math.random() * 5000); // do the userSelect task filtering devices by the criteria_json parameter
                            if (_fail(0.5)) {
                                // @ts-ignore
                    ...
                    case "autoSelect": // params: (criteria_json, scan_period_number, timeout_number)
                        {
                            ...
                            await (0, functions_js_1.sleep)(Math.random() * 5000); // do the autoSelect task filtering devices by the criteria_json parameter and scanning minimum time scan_period_number, maximum timeout_number
                            if (_fail(0.1)) {
                                // @ts-ignore
                    ...
                    case "unselect":
                        {
                            ...
                            await (0, functions_js_1.sleep)(10); // unselect logic
                            _selected = false;
                            // @ts-ignore
                    ...
                    case "connect":
                        {
                            ...
                            await (0, functions_js_1.sleep)(Math.random() * 5000); // connecting logic
                            if (_fail(0.1)) {
                                // @ts-ignore
                            ...
                            await (0, functions_js_1.sleep)(1000); // unselect logic
                            // @ts-ignore
                    ...
                    case "disconnect":
                        {
                            // params: ()
                            if (_connected) {
                                await (0, functions_js_1.sleep)(100); // disconnecting logic
                                _connected = false;
                                // @ts-ignore
                                window.flutterConnection.emit("#disconnected");
                            }
                    ...
                    case "deliver":
                        {
                            ...
                            await (0, functions_js_1.sleep)(25); // delivering logic
                            if (_fail(0.1)) {
                                // @ts-ignore
                    ...
                    case "transmit":
                        {
                            ...
                            await (0, functions_js_1.sleep)(10); // transmiting logic
                            if (_fail(0.1)) {
                                // @ts-ignore
                    ...
                    case "request":
                        {
                            ...
                            await (0, functions_js_1.sleep)(50); // requesting logic
                            if (_fail(0.1)) {
                                // @ts-ignore
                    ...
                    case "writeClock":
                        {
                            ...
                            await (0, functions_js_1.sleep)(10); // writing clock logic.
                            if (_fail(0.1)) {
                                // @ts-ignore
                    ...
                    case "readClock":
                        {
                            ...
                            await (0, functions_js_1.sleep)(50); // reading clock logic.
                            if (_fail(0.1)) {
                                // @ts-ignore
                    ...
                    case "updateFW":
                        {
                            ...
                            await (0, functions_js_1.sleep)(10000); // preparing FW logic.
                            if (_fail(0.1)) {
                                // @ts-ignore
                            ...
                            for (let i = 1; i <= 100; i++) {
                                ...
                                await (0, functions_js_1.sleep)(25); // writing FW logic.
                                if (_fail(0.01)) {
                                    // @ts-ignore
                                    window.flutterConnection.emit("ota_status", "fail");
                                    // @ts-ignore
                            ...
                            await (0, functions_js_1.sleep)(1000); // finishing FW logic.
                            if (_fail(0.1)) {
                                // @ts-ignore
                                window.flutterConnection.emit("ota_status", "fail");
                                // @ts-ignore
                        ...
                        break;
                    default:
                        Logging_js_1.logging.error("Unknown handler");
                        break;
                }
            };
        }
...
class FlutterConnector extends FlutterConnection {
    #interfaceReference;
    #promise;
  ...
  */
    ...
    userSelect(criteria_object, timeout_number = 60000) {
        ...
        Logging_js_1.logging.debug(`userSelect(criteria=${criteria_json}, timeout=${timeout_number})`);
        this.#promise = new Promise((resolve, reject) => {
            // @ts-ignore
    ...
    autoSelect(criteria_object, scan_period_number = 1000, timeout_number = 10000) {
        ...
        //         then return error
        const criteria_json = JSON.stringify(criteria_object);
        Logging_js_1.logging.debug(`autoSelect(criteria=${criteria_json}, scan_period=${scan_period_number}, timeout=${timeout_number})`);
        this.#promise = new Promise((resolve, reject) => {
            // @ts-ignore
            window.flutterConnection.resolve = function (j) {
                resolve(j ? JSON.parse(j) : null);
    ...
    scan(criteria_object, scan_period_number = 5000) {
        // step 1. for the scan_period scan the surroundings for BLE devices.
        const criteria_json = JSON.stringify(criteria_object);
        Logging_js_1.logging.debug(`scan(criteria=${criteria_json}, scan_period=${scan_period_number})`);
        this.#promise = new Promise((resolve, reject) => {
            // @ts-ignore
            window.flutterConnection.resolve = function (j) {
                resolve(j ? JSON.parse(j) : null);
    ...
    */
    connect(timeout_number = 10000) {
        Logging_js_1.logging.debug(`connect(timeout=${timeout_number})`);
        if (timeout_number <= 0) {
        ...
        return this.#applyTimeout(this.#promise, timeout_number < 5000 ? 10000 : timeout_number * 2, "connect").then(() => {
            Logging_js_1.logging.debug("Sleeping for 200ms");
            return (0, functions_js_1.sleep)(200);
        });
    }
    ...
    deliver(payload_bytes, timeout_number = 5000) {
        Logging_js_1.logging.debug(`deliver(payload=[${payload_bytes}], timeout=${timeout_number})`);
        this.#promise = new Promise((resolve, reject) => {
            // @ts-ignore
            window.flutterConnection.resolve = resolve;
            // @ts-ignore
    ...
    transmit(payload_bytes, timeout_number = 1000) {
        Logging_js_1.logging.debug(`transmit(payload=[${payload_bytes}], timeout=${timeout_number})`);
        this.#promise = new Promise((resolve, reject) => {
            // @ts-ignore
            window.flutterConnection.resolve = resolve;
            // @ts-ignore
    ...
    request(payload_bytes, read_response = true, timeout_number = 5000) {
        Logging_js_1.logging.debug(`request(payload=[${payload_bytes}], read_response=${read_response ? "true" : "false"}, timeout=${timeout_number})`);
        this.#promise = new Promise((resolve, reject) => {
            // @ts-ignore
    ...
    setClock(clock) {
        Logging_js_1.logging.debug("setClock()");
        return new Promise(async (resolve, reject) => {
            for (let index = 0; index < 3; index++) {
                ...
                try {
                    ...
                    const clock_bytes = (0, functions_js_1.toBytes)(timestamp, 8);
                    // @ts-ignore
                    window.flutter_inappwebview.callHandler("writeClock", clock_bytes);
                    await this.#applyTimeout(this.#promise, 5000, "writeClock");
                    Logging_js_1.logging.debug("Clock write success:", timestamp);
                    // @ts-ignore
                    resolve();
                    return;
                }
                catch (e) {
                    Logging_js_1.logging.warn("Clock write failed: " + e);
                }
            }
        ...
        return new Promise(async (resolve, reject) => {
            for (let index = 0; index < 3; index++) {
                try {
                    ...
                    const reader = new TnglReader_js_1.TnglReader(new DataView(new Uint8Array(bytes).buffer));
    ...
    updateFW(firmware_bytes) {
        Logging_js_1.logging.debug(`updateFW(firmware_bytes.length=${firmware_bytes.length})`);
        // this.#promise = new Promise((resolve, reject) => {
        //   // @ts-ignore
        //   window.flutterConnection.resolve = resolve;
        //   // @ts-ignore
        ...
        return new Promise(async (resolve, reject) => {
            const chunk_size = (0, functions_js_1.detectAndroid)() ? 480 : 3984; // must be modulo 16
            // const chunk_size = 992; // must be modulo 16
            let index_from = 0;
            let index_to = chunk_size;
            let written = 0;
            Logging_js_1.logging.info("OTA UPDATE");
            Logging_js_1.logging.verbose(firmware_bytes);
            ...
            try {
                this.#interfaceReference.emit("ota_status", "begin");
                {
                    ...
                    const device_bytes = [SpectodaInterfaceLegacy_js_1.COMMAND_FLAGS.FLAG_OTA_RESET, 0x00, ...(0, functions_js_1.numberToBytes)(0x00000000, 4)];
                ...
                await (0, functions_js_1.sleep)(100);
                {
                    ...
                    const device_bytes = [SpectodaInterfaceLegacy_js_1.COMMAND_FLAGS.FLAG_OTA_BEGIN, 0x00, ...(0, functions_js_1.numberToBytes)(firmware_bytes.length, 4)];
                    await this.request(device_bytes, false, 20000);
                }
                await (0, functions_js_1.sleep)(8000);
                {
                    //===========// WRITE //===========//
                    ...
                    while (written < firmware_bytes.length) {
                        ...
                        const device_bytes = [SpectodaInterfaceLegacy_js_1.COMMAND_FLAGS.FLAG_OTA_WRITE, 0x00, ...(0, functions_js_1.numberToBytes)(written, 4), ...firmware_bytes.slice(index_from, index_to)];
                        await this.request(device_bytes, false);
                ...
                await (0, functions_js_1.sleep)(100);
                {
                    ...
                    const device_bytes = [SpectodaInterfaceLegacy_js_1.COMMAND_FLAGS.FLAG_OTA_END, 0x00, ...(0, functions_js_1.numberToBytes)(written, 4)];
                    await this.request(device_bytes, false);
                }
                await (0, functions_js_1.sleep)(100);
                Logging_js_1.logging.info("Rebooting device...");
                const device_bytes = [SpectodaInterfaceLegacy_js_1.COMMAND_FLAGS.FLAG_DEVICE_REBOOT_REQUEST];
                await this.request(device_bytes, false);
                Logging_js_1.logging.debug("Firmware written in " + (new Date().getTime() - start_timestamp) / 1000 + " seconds");
...
exports.FlutterConnector = FlutterConnector;

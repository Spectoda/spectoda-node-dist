...
class NodeBLEConnection {
    ...
    attach(service, networkUUID, clockUUID, deviceUUID) {
        ...
        return this.#service
            ...
            .then(characteristic => {
            Logging_js_1.logging.debug("#networkChar", characteristic);
            ...
            .then(characteristic => {
            Logging_js_1.logging.debug("#clockChar", characteristic);
            ...
            .then(characteristic => {
            Logging_js_1.logging.debug("#deviceChar", characteristic);
  ...
  */
    ...
    async autoSelect(criteria, scanPeriod, timeout) {
        ...
        try {
            ...
            if (!await this.#bluetoothAdapter.isDiscovering()) {
                Logging_js_1.logging.debug("> Starting BLE scanner");
                await this.#bluetoothAdapter.startDiscovery();
            }
            else {
                Logging_js_1.logging.debug("> Restarting BLE scanner");
            ...
            Logging_js_1.logging.debug("> Getting BLE device mac address");
            const mac = await this.#bluetoothDevice.getAddress().catch(e => console.error(e));
            Logging_js_1.logging.debug("> Getting BLE device name");
    ...
    async scan(criteria, scanPeriod = 10000) {
        ...
        try {
            ...
            // this.#criteria = criteria;
            if (await this.connected()) {
                Logging_js_1.logging.debug("> Disconnecting device");
                await this.disconnect().catch(e => Logging_js_1.logging.error(e));
                await (0, functions_js_1.sleep)(1000);
            }
            if (!this.#bluetoothAdapter) {
                Logging_js_1.logging.debug("> Requesting default bluetooth adapter");
                this.#bluetoothAdapter = await this.#bluetooth.defaultAdapter();
            }
            if (!await this.#bluetoothAdapter.isDiscovering()) {
                Logging_js_1.logging.debug("> Starting BLE scanner");
            ...
            Logging_js_1.logging.debug("> Devices Scanned:", devices);
    ...
    async connect(timeout = 60000) {
        ...
        Logging_js_1.logging.debug("> Getting the GATT server...");

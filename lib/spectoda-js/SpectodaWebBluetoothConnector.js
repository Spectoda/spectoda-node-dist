...
exports.SpectodaWebBluetoothConnector = exports.WebBLEConnection = void 0;
...
class WebBLEConnection {
    ...
    #writeBytes(characteristic, bytes, response) {
        ...
        const packet_size = (0, functions_js_1.detectAndroid)() ? 212 : 512; // min size packet_header_size + 1 !!!! ANDROID NEEDS PACKET SIZE <= 212!!!!
    ...
    #onNetworkNotification(event) {
        Logging_js_1.logging.verbose("#onNetworkNotification", event);
    ...
    #onDeviceNotification(event) {
        Logging_js_1.logging.verbose("#onDeviceNotification", event);
        // let value = event.target.value;
        // let a = [];
        // for (let i = 0; i < value.byteLength; i++) {
        //   a.push("0x" + ("00" + value.getUint8(i).toString(16)).slice(-2));
    ...
    #onClockNotification(event) {
        Logging_js_1.logging.verbose("#onClockNotification", event);
        // let value = event.target.value;
        // let a = [];
        // for (let i = 0; i < value.byteLength; i++) {
        //   a.push("0x" + ("00" + value.getUint8(i).toString(16)).slice(-2));
    ...
    attach(service, networkUUID, clockUUID, deviceUUID) {
        ...
        Logging_js_1.logging.debug("> Getting Network Characteristics...");
        return this.#service
            ...
            return this.#networkChar
                .startNotifications()
                .then(() => {
                Logging_js_1.logging.debug("> Network notifications started");
            ...
            .then(() => {
            Logging_js_1.logging.debug("> Getting Clock Characteristics...");
            ...
            return this.#clockChar
                .startNotifications()
                .then(() => {
                Logging_js_1.logging.debug("> Clock notifications started");
            ...
            .then(() => {
            Logging_js_1.logging.debug("> Getting Device Characteristics...");
            ...
            return this.#deviceChar
                .startNotifications()
                .then(() => {
                Logging_js_1.logging.debug("> Device notifications started");
    ...
    writeClock(timestamp) {
        ...
        this.#writing = true;
        const bytes = (0, functions_js_1.toBytes)(timestamp, 8);
        return this.#clockChar
            .writeValueWithoutResponse(new Uint8Array(bytes))
            .then(() => {
            Logging_js_1.logging.debug("Clock characteristics written");
        })
            .catch(e => {
            Logging_js_1.logging.error(e);
    ...
    updateFirmware(firmware) {
        ...
        this.#writing = true;
        return new Promise(async (resolve, reject) => {
            const chunk_size = (0, functions_js_1.detectAndroid)() ? 1008 : 4992; // must be modulo 16
            let index_from = 0;
            ...
            try {
                this.#interfaceReference.emit("ota_status", "begin");
                {
                    ...
                    const bytes = [SpectodaInterfaceLegacy_js_1.COMMAND_FLAGS.FLAG_OTA_RESET, 0x00, ...(0, functions_js_1.numberToBytes)(0x00000000, 4)];
                ...
                await (0, functions_js_1.sleep)(100);
                {
                    ...
                    const bytes = [SpectodaInterfaceLegacy_js_1.COMMAND_FLAGS.FLAG_OTA_BEGIN, 0x00, ...(0, functions_js_1.numberToBytes)(firmware.length, 4)];
                    await this.#writeBytes(this.#deviceChar, bytes, true);
                }
                await (0, functions_js_1.sleep)(8000); // need to wait 10 seconds to let the ESP erase the flash.
                {
                    ...
                    while (written < firmware.length) {
                        ...
                        const bytes = [SpectodaInterfaceLegacy_js_1.COMMAND_FLAGS.FLAG_OTA_WRITE, 0x00, ...(0, functions_js_1.numberToBytes)(written, 4), ...firmware.slice(index_from, index_to)];
                        await this.#writeBytes(this.#deviceChar, bytes, true);
                ...
                await (0, functions_js_1.sleep)(100);
                {
                    ...
                    const bytes = [SpectodaInterfaceLegacy_js_1.COMMAND_FLAGS.FLAG_OTA_END, 0x00, ...(0, functions_js_1.numberToBytes)(written, 4)];
                    await this.#writeBytes(this.#deviceChar, bytes, true);
                }
...
exports.WebBLEConnection = WebBLEConnection;
/////////////////////////////////////////////////////////////////////////////////////
...
class SpectodaWebBluetoothConnector {
    #interfaceReference;
    #webBTDevice;
    #connection;
    #reconection;
  ...
  */
    ...
    userSelect(criteria, timeout) {
        ...
        if (this.#connected()) {
            return this.disconnect()
                .then(() => {
                return (0, functions_js_1.sleep)(1000);
            })
                .then(() => {
            ...
            for (let i = 0; i < this.#criteria.length; i++) {
                ...
                if (criterium.fwVersion || criterium.ownerSignature || criterium.productCode || criterium.adoptionFlag) {
                    ...
                    if (criterium.ownerSignature) {
                        ...
                        const owner_signature_code_bytes = (0, functions_js_1.hexStringToUint8Array)(criterium.ownerSignature);
                        for (let i = 0; i < 16; i++) {
    ...
    connect(timeout = 10000) {
        ...
        if (timeout <= 0) {
            Logging_js_1.logging.debug("> Connect timeout have expired");
        ...
        if (this.#connected()) {
            Logging_js_1.logging.debug("> Bluetooth Device is already connected");
            return Promise.resolve();
        }
        const timeout_handle = setTimeout(() => {
            Logging_js_1.logging.warn("Timeout triggered");
            this.disconnect();
        }, timeout < 10000 ? 10000 : timeout);
        Logging_js_1.logging.debug("> Connecting to Bluetooth device...");
        return this.#webBTDevice.gatt
            .connect()
            .then(server => {
            this.#connection.reset();
            Logging_js_1.logging.debug("> Getting the Bluetooth Service...");
            return server.getPrimaryService(this.SPECTODA_SERVICE_UUID);
        })
            .then(service => {
            Logging_js_1.logging.debug("> Getting the Service Characteristic...");
            clearTimeout(timeout_handle);
            return this.#connection.attach(service, this.TERMINAL_CHAR_UUID, this.CLOCK_CHAR_UUID, this.DEVICE_CHAR_UUID);
        })
            .then(() => {
            Logging_js_1.logging.debug("> Bluetooth Device Connected");
            ...
            .catch(error => {
            Logging_js_1.logging.warn(error.name);
            clearTimeout(timeout_handle);
            // If the device is far away, sometimes this "NetworkError" happends
            if (error.name == "NetworkError") {
                return (0, functions_js_1.sleep)(1000).then(() => {
                    if (this.#reconection) {
    ...
    disconnect() {
        this.#reconection = false;
        Logging_js_1.logging.debug("> Disconnecting from Bluetooth Device...");
        this.#connection.reset();
        if (this.#connected()) {
            this.#disconnect();
        }
    ...
    #onDisconnected = event => {
        Logging_js_1.logging.debug("> Bluetooth Device disconnected");
        this.#connection.reset();
        if (this.#connectedGuard) {
    ...
    setClock(clock) {
        Logging_js_1.logging.verbose("setClock()");
        if (!this.#connected()) {
            return Promise.reject("DeviceDisconnected");
        }
        return new Promise(async (resolve, reject) => {
...
exports.SpectodaWebBluetoothConnector = SpectodaWebBluetoothConnector;

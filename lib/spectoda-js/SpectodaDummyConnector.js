...
exports.SpectodaDummyConnector = void 0;
...
const TnglWriter_js_1 = require("./TnglWriter.js");
/////////////////////////////////////////////////////////////////////////////////////
...
class SpectodaDummyConnector {
    ...
    constructor(interfaceReference, enableErrors = false, dummyFWVersion = "DUMMY_0.0.0_00000000") {
        ...
        this.#clock = new TimeTrack_js_1.TimeTrack(0, false);
    }
    #fail(chance) {
  ...
  */
    ...
    userSelect(criteria) {
        Logging_js_1.logging.verbose("userSelect(criteria=", criteria, ")");
        return new Promise(async (resolve, reject) => {
            if (this.#connected) {
                await this.disconnect();
            }
            await (0, functions_js_1.sleep)(Math.random() * 1000); // userSelect logic
            if (this.#fail(0.25)) {
    ...
    autoSelect(criteria, scan_period, timeout) {
        Logging_js_1.logging.verbose("autoSelect(criteria=", criteria, ", scan_period=", scan_period, "timeout=", timeout, ")");
        ...
        return new Promise(async (resolve, reject) => {
            ...
            await (0, functions_js_1.sleep)(Math.random() * 1000); // autoSelect logic
            if (this.#fail(0.1)) {
                reject("SelectionFailed");
                return;
            }
    ...
    selected() {
        Logging_js_1.logging.verbose(`selected()`);
        return new Promise(async (resolve, reject) => {
            if (this.#selected) {
                resolve({ connector: this.type });
            }
    ...
    unselect() {
        Logging_js_1.logging.verbose(`unselect()`);
        return new Promise(async (resolve, reject) => {
            if (this.#connected) {
                await this.disconnect();
            }
    ...
    connect(timeout) {
        Logging_js_1.logging.verbose(`connect(timeout=${timeout})`);
        return new Promise(async (resolve, reject) => {
            if (!this.#selected) {
                reject("DeviceNotSelected");
                return;
            }
            await (0, functions_js_1.sleep)(Math.random() * 1000); // connecting logic
            if (this.#fail(0.1)) {
    ...
    disconnect() {
        Logging_js_1.logging.verbose(`disconnect()`);
        return new Promise(async (resolve, reject) => {
            if (this.#connected) {
    ...
    connected() {
        Logging_js_1.logging.verbose(`connected()`);
        return new Promise(async (resolve, reject) => {
            if (this.#connected) {
                resolve({ connector: this.type });
            }
    ...
    deliver(payload, timeout) {
        Logging_js_1.logging.verbose(`deliver(payload=${payload}, timeout=${timeout})`);
        return new Promise(async (resolve, reject) => {
            if (!this.#connected) {
    ...
    transmit(payload, timeout) {
        Logging_js_1.logging.verbose(`transmit(payload=${payload}, timeout=${timeout})`);
        return new Promise(async (resolve, reject) => {
            if (!this.#connected) {
                reject("DeviceDisconnected");
                return;
    ...
    request(payload, read_response = true, timeout) {
        Logging_js_1.logging.verbose(`request(payload=${payload}, read_response=${read_response ? "true" : "false"}, timeout=${timeout})`);
        ...
        return new Promise(async (resolve, reject) => {
            ...
            let reader = new TnglReader_js_1.TnglReader(new DataView(new Uint8Array(payload).buffer));
            switch (reader.peekFlag()) {
                case SpectodaInterfaceLegacy_js_1.COMMAND_FLAGS.FLAG_ADOPT_REQUEST:
                    {
                        ...
                        let writer = new TnglWriter_js_1.TnglWriter(64);
                        writer.writeFlag(SpectodaInterfaceLegacy_js_1.COMMAND_FLAGS.FLAG_ADOPT_RESPONSE);
                    ...
                    break;
                case SpectodaInterfaceLegacy_js_1.COMMAND_FLAGS.FLAG_CONFIG_UPDATE_REQUEST:
                    {
                        ...
                        // log_d("error_code=%u", error_code);
                        let writer = new TnglWriter_js_1.TnglWriter(64);
                        writer.writeFlag(SpectodaInterfaceLegacy_js_1.COMMAND_FLAGS.FLAG_CONFIG_UPDATE_RESPONSE);
                        writer.writeUint32(request_uuid);
                        writer.writeUint8(error_code);
                        resolve(writer.bytes);
                    }
                    break;
                case SpectodaInterfaceLegacy_js_1.COMMAND_FLAGS.FLAG_TIMELINE_REQUEST:
                    {
                        ...
                        let writer = new TnglWriter_js_1.TnglWriter(64);
                        writer.writeFlag(SpectodaInterfaceLegacy_js_1.COMMAND_FLAGS.FLAG_TIMELINE_RESPONSE);
                        writer.writeUint32(request_uuid);
                    ...
                    break;
                case SpectodaInterfaceLegacy_js_1.COMMAND_FLAGS.FLAG_TNGL_FINGERPRINT_REQUEST:
                    {
                        ...
                        // log_d("error_code=%u", error_code);
                        let writer = new TnglWriter_js_1.TnglWriter(64);
                        writer.writeFlag(SpectodaInterfaceLegacy_js_1.COMMAND_FLAGS.FLAG_TNGL_FINGERPRINT_RESPONSE);
                        writer.writeUint32(request_uuid);
                        writer.writeUint8(error_code);
                        if (error_code == ERROR_CODE_SUCCESS) {
                            writer.writeBytes(new Uint8Array([0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]), 32);
                        }
                        resolve(writer.bytes);
                    }
                    break;
                case SpectodaInterfaceLegacy_js_1.COMMAND_FLAGS.FLAG_ERASE_OWNER_REQUEST:
                    {
                        ...
                        // log_d("error_code=%u", error_code);
                        let writer = new TnglWriter_js_1.TnglWriter(64);
                        writer.writeFlag(SpectodaInterfaceLegacy_js_1.COMMAND_FLAGS.FLAG_ERASE_OWNER_RESPONSE);
                        writer.writeUint32(request_uuid);
                        writer.writeUint8(error_code);
                        // mac address
                        if (error_code == ERROR_CODE_SUCCESS) {
                    ...
                    break;
                case SpectodaInterfaceLegacy_js_1.COMMAND_FLAGS.FLAG_FW_VERSION_REQUEST:
                    {
                        ...
                        // log_d("error_code=%u", error_code);
                        let writer = new TnglWriter_js_1.TnglWriter(64);
                        writer.writeFlag(SpectodaInterfaceLegacy_js_1.COMMAND_FLAGS.FLAG_FW_VERSION_RESPONSE);
                        writer.writeUint32(request_uuid);
                        writer.writeUint8(error_code);
    ...
    setClock(clock) {
        Logging_js_1.logging.verbose(`setClock(clock.millis()=${clock.millis()})`);
        return new Promise(async (resolve, reject) => {
            if (!this.#connected) {
                reject("DeviceDisconnected");
                return;
    ...
    getClock() {
        Logging_js_1.logging.verbose(`getClock()`);
        return new Promise(async (resolve, reject) => {
            if (!this.#connected) {
                reject("DeviceDisconnected");
                return;
    ...
    // to all handlers
    updateFW(firmware) {
        Logging_js_1.logging.verbose(`updateFW(firmware=${firmware})`);
        return new Promise(async (resolve, reject) => {
            if (!this.#connected) {
                reject("DeviceDisconnected");
                return;
...
exports.SpectodaDummyConnector = SpectodaDummyConnector;
